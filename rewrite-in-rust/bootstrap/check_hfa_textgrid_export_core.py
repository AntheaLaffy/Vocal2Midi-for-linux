"""Check HubertFA TextGrid export fixtures against legacy Python."""
...
FIXTURE_PATH = REWRITE_ROOT / "fixtures" / "hfa_textgrid_export_core.jsonl"
...
def plan_with_legacy(case: dict[str, Any]) -> dict[str, Any]:
    ...
    def fake_codecs_open(
        ...
        if mode != "w":
            return original_codecs_open(filename, mode, encoding, errors, buffering)
        return FakeWriter(pathlib.Path(filename), encoding, writes)
...
def main() -> None:
    ...
    print(f"validated {len(cases)} hfa_textgrid_export_core fixtures")
