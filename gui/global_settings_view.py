...
from gui.settings_utils import migrate_legacy_qsettings


class GlobalSettingsInterface(ScrollArea):
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        default_asr_model = "experiments/Qwen3-ASR-1.7B-dml"
        if platform.system().lower() != "windows":
            default_asr_model = "experiments/Qwen3-ASR-1.7B"
        self.project_root = pathlib.Path(__file__).resolve().parent.parent
        settings_path = migrate_legacy_qsettings(self.project_root)
