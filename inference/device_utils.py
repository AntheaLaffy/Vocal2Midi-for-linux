...
_IS_WINDOWS = _SYSTEM_NAME == "windows"

RUNTIME_DEVICE_CHOICES = ("dml", "cpu", "cuda")
VISIBLE_RUNTIME_DEVICE_CHOICES = ("dml", "cpu") if _IS_WINDOWS else ("cpu",)
...
def normalize_runtime_device(device: str | None, default: str | None = None) -> str:
    if default is None:
        default = "dml" if _IS_WINDOWS else "cpu"
...
def _call_com_method(obj: ctypes.c_void_p, method_index: int, restype, argtypes, *args):
    ...
    func_type = ctypes.WINFUNCTYPE if hasattr(ctypes, "WINFUNCTYPE") else ctypes.CFUNCTYPE
    func = func_type(restype, *argtypes)(func_ptr)
    return func(*args)
