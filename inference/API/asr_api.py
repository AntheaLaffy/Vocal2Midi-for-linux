...
DEFAULT_QWEN_ASR_TIMEOUT_SEC = 180 if _IS_WINDOWS else 600
...
class Qwen3ASROfficialModel:
    """Thin adapter for the official qwen-asr Transformers backend."""

    def __init__(self, model_path, device="cpu", max_inference_batch_size=1, max_new_tokens=256):
        import torch
    ...
    def transcribe(self, audio, language: str | None = None, context: str | None = None, batch_size: int | None = None):
        if isinstance(audio, (str, Path)):
            return self.model.transcribe(audio=str(audio), language=language, context=context or "")
        if isinstance(audio, Iterable):
            audio_files = [str(path) for path in audio]
            if not audio_files:
                return []
            return self.model.transcribe(audio=audio_files, language=language, context=context or "")
...
def _load_qwen_dml_model(model_path, requested_device):
...
def _load_qwen_official_model(model_path, requested_device):
    model = Qwen3ASROfficialModel(
        model_path,
        device="cpu" if requested_device == "dml" else requested_device,
        max_inference_batch_size=1,
        max_new_tokens=256,
    )
    print(f"Qwen ASR official runtime ready: backend=qwen-asr transformers, device={model.device}.")
    return model


def load_qwen_model(model_path, device="dml", use_cache=True):
    """
    Loads the Qwen ASR model.

    Windows uses the project-local ONNX/DML + llama.cpp compatibility runtime.
    Linux/macOS use the official qwen-asr Transformers backend, which does not
    require llama.cpp shared libraries.

    Caches model in-process to avoid repeated loading.
    """
    requested_device = normalize_runtime_device(device)
    backend = "dml" if _IS_WINDOWS else "official"
    cache_key = (str(model_path), requested_device, backend)
    if use_cache:
        with _QWEN_MODEL_CACHE_LOCK:
            cached_model = _QWEN_MODEL_CACHE.get(cache_key)
        if cached_model is not None:
            print(f"Reusing cached Qwen ASR model from '{model_path}' on {requested_device} ({backend}).")
            return cached_model

    try:
        print(f"Loading Qwen ASR runtime from '{model_path}' (requested device: {requested_device}, backend: {backend})...")
        if _IS_WINDOWS:
            model = _load_qwen_dml_model(model_path, requested_device)
        else:
            model = _load_qwen_official_model(model_path, requested_device)
    except Exception as e:
        raise RuntimeError(
            f"Error loading Qwen ASR runtime: {e}\n"
            "On Windows, ensure the DML/GGUF model files are present. "
            "On Linux/macOS, install qwen-asr and use the official Transformers model directory."
        )
...
def _asr_worker_main(model_path, device, task_queue, result_queue):
    """Runs a single non-daemon ASR worker process for the active Qwen backend."""
    model = None
    try:
        proc_name = mp.current_process().name
        print(f"Initializing ASR worker ({proc_name}) with model '{model_path}' on {device}...")
...
def batch_transcribe_asr(
    ...
    backend = "DML/GGUF" if _IS_WINDOWS else "official qwen-asr"
    print(f"[ASR API] Running ASR with Qwen {backend} runtime (Batch Size: {asr_batch_size}, Language: {asr_lang})...")
